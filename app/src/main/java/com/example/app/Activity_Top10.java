...
public class Activity_Top10 extends AppCompatActivity implements OnMapReadyCallback{
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if(!itemDataList.get(0).getxText().equals("long")) {
            double la = Double.parseDouble(itemDataList.get(0).getxText());
            double ln = Double.parseDouble(itemDataList.get(0).getyText());
            googleMap.addMarker(new MarkerOptions().position(new LatLng(la, ln)).title("winner"));
        }
        if(!itemDataList.get(1).getxText().equals("long")){
            double la = Double.parseDouble(itemDataList.get(1).getxText());
            double ln = Double.parseDouble(itemDataList.get(1).getyText());
            googleMap.addMarker(new MarkerOptions().position(new LatLng(la,ln)).title("winner 1"));
        }
        if(!itemDataList.get(2).getxText().equals("long")){
            double la = Double.parseDouble(itemDataList.get(2).getxText());
            double ln = Double.parseDouble(itemDataList.get(2).getyText());
            googleMap.addMarker(new MarkerOptions().position(new LatLng(la,ln)).title("winner 2"));
        }
